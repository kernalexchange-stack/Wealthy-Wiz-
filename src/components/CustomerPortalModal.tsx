...
export const CustomerPortalModal: React.FC<CustomerPortalModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-3 sm:p-4 overflow-y-auto animate-in fade-in duration-200">
      <div className="bg-[#1e2945] border border-[#3b4f7e] text-slate-100 rounded-3xl max-w-4xl w-full max-h-[92vh] flex flex-col shadow-2xl relative overflow-hidden">
        ...
        {/* Tab Controls */}
        <div className="px-6 border-b border-[#334674] bg-[#18233e] flex items-center gap-2">
          <button
          ...
          {activeTab === 'hub' && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="p-4 bg-[#24345c] border border-[#384e84] rounded-2xl space-y-1 shadow-sm">
                  <div className="text-[10px] uppercase font-bold text-slate-300">Investor Status</div>
                  <div className="text-base font-bold text-white flex items-center gap-1.5">
                    <CheckCircle2 className="w-4 h-4 text-cyan-400" />
                    <span>KYC & PAN Verified</span>
                  </div>
                  <div className="text-[11px] text-slate-300 font-mono">PAN: {currentUser.panNumber || 'ABCDE1234F'}</div>
                </div>

                <div className="p-4 bg-[#24345c] border border-[#384e84] rounded-2xl space-y-1 shadow-sm">
                  <div className="text-[10px] uppercase font-bold text-slate-300">Risk Profile</div>
                ...
                <div className="p-4 bg-[#24345c] border border-[#384e84] rounded-2xl space-y-1 shadow-sm">
                  <div className="text-[10px] uppercase font-bold text-slate-300">Assigned Advisor</div>
                  <div className="text-base font-bold text-white flex items-center gap-1.5">
              ...
              <div className="p-5 bg-gradient-to-r from-cyan-900/40 via-[#24345c] to-[#2b3d6c] border border-cyan-700/50 rounded-2xl space-y-3 shadow-sm">
                ...
                <div className="grid grid-cols-3 gap-2 pt-2">
                  <div className="bg-[#19233f] p-3 rounded-xl border border-cyan-500/30 text-center shadow-sm">
                    ...
                    <div className="text-[10px] text-emerald-400 font-medium">✓ Completed</div>
                  </div>

                  <div className="bg-[#19233f] p-3 rounded-xl border border-amber-500/30 text-center shadow-sm">
                    ...
                    <div className="text-[10px] text-amber-300 font-medium">● Curated Live</div>
                  </div>

                  <div className="bg-[#19233f] p-3 rounded-xl border border-[#384e84] text-center opacity-85 shadow-sm">
                    <div className="w-6 h-6 rounded-full bg-slate-700 text-slate-300 font-bold text-xs flex items-center justify-center mx-auto mb-1">3</div>
                    <div className="text-xs font-bold text-slate-200">SIP Execution</div>
                    <div className="text-[10px] text-slate-400">Auto-Debit Ready</div>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <button
                  ...
                  className="p-4 bg-[#24345c] hover:bg-[#2d4070] border border-[#384e84] rounded-2xl text-left transition-all group flex items-start justify-between shadow-sm"
                >
                  <div>
                  ...
                  <ArrowRight className="w-4 h-4 text-slate-400 group-hover:text-cyan-300 transition-transform group-hover:translate-x-1" />
                </button>
                ...
                >
                  ...
                  <ArrowRight className="w-4 h-4 text-slate-400 group-hover:text-amber-300 transition-transform group-hover:translate-x-1" />
                </button>
              </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-slate-300">Live official AMFI daily NAV tracking</p>
                </div>
                <button
                  onClick={() => {
                    onClose();
              ...
              <div className="space-y-3">
                {watchlistFunds.map((fund) => (
                  <div
                    key={fund.schemeCode}
                    className="p-4 bg-[#24345c] border border-[#384e84] rounded-2xl flex flex-wrap items-center justify-between gap-3 shadow-sm"
                  >
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] text-slate-300 font-mono">AMFI: {fund.schemeCode}</span>
                      </div>
                      <h4 className="text-sm font-bold text-white mt-1">{fund.schemeName}</h4>
                      <div className="text-xs text-slate-300">{fund.fundHouse}</div>
                    </div>
          ...
          {activeTab === 'requests' && (
            <div className="space-y-4">
              ...
              {customerLeads.length === 0 ? (
                <div className="p-8 text-center bg-[#24345c] border border-[#384e84] rounded-2xl text-slate-300 space-y-3 shadow-sm">
                  ...
                  <button
                    ...
                    className="bg-gradient-to-r from-[#fbbf24] to-[#f59e0b] text-slate-950 font-bold text-xs px-4 py-2 rounded-xl shadow"
                  >
                ...
                customerLeads.map((req) => (
                  <div
                    ...
                    className="p-5 bg-[#24345c] border border-[#384e84] rounded-2xl space-y-3 shadow-sm"
                  >
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div>
                        <span className="text-xs font-mono font-bold text-cyan-300">{req.id || 'ADVISORY-REQ'}</span>
                    ...
                    <div className="text-xs text-slate-200 flex flex-wrap gap-4 font-mono">
